...
public class NSGA2UniquePopulation<T extends Chromosome & MultiObjective> implements IMultiObjOptAlgorithm<T> {
	...
	public NSGA2UniquePopulation(PopulationGenerator<T> populationGenerator, Mutation<T> mutation,
			Crossover<T> crossover, Evaluator<T> evaluator, int maxNumberOfIterations, int sizeOfPopulation, int k) {
		super();
	...
	public List<T> runAndReturnFinishingPopualtion() {
		...
		Set<T> populationSet = new HashSet<>();
		...
		for (int i = 0; i < sizeOfPopulation; i++) {
			...
			populationSet.add(population[i]);
		}
		...
		for (int i = 0; i < maxNumberOfIterations; i++) {
			...
			// TODO remove this
			if (i % 100 == 0) {
				System.out.format("Pareto front at iteration %d:%n", i);
				for (int index : sortedPop.get(0)) {
					System.out.println(population[index].toString());
				}
				if (sortedPop.size() > 1) {
					System.out.println("Second front:");
					for (int index : sortedPop.get(1)) {
						System.out.println(population[index].toString());
					}
				}
			}
			...
			while (totalSize < union.length) {
				...
				for (int l = 0; l < children.length && totalSize < union.length; l++) {
					...
					if (populationSet.contains(children[l])) {
						continue;
					}
					populationSet.add(children[l]);
